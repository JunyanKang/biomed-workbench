...
For long-running projects, established sample information, reference releases, cell annotation, statistical units, analysis environments, colours and formal figures continue across analysis rounds. Before repeating an analysis, the workbench checks its recorded Conda or other runtime environment: content-equivalent environments are reused, while drift stops the computation until the original environment is restored or a new analysis branch is approved. Data, plot-ready tables, figures, captions, prose and literature sources can connect to the same [scientific evidence map](docs/scientific-evidence-map.md), so the origin of each conclusion remains clear. A formal project analysis report uses a navigable HTML page with evidence links as its reading entry; Markdown is retained only as a searchable companion and cannot establish delivery by itself. Every scientific-evidence-map version also provides Chinese and English HTML views that link directly to registered data, analysis scripts, final figures, captions, and original studies. Conflicting and negative findings stay visible alongside supportive results and help determine whether the project should proceed, change direction or acquire another form of evidence.
...
- **Manuscripts and reports:** first use the study design, project results and verified literature to decide whether each item contributes discovery, context, mechanistic consistency, orthogonal validation or an evidentiary boundary. Titles, abstracts, Results, Discussion, Methods and captions are then built around that scientific argument. A dedicated language pass replaces engineering, computer-system and internal process language that does not belong in biomedical prose while preserving necessary method names, numbers, citations, terminology, uncertainty and claim strength. Final text is accompanied by an HTML reading page that is written and reopened before delivery, with direct navigation among the argument, source and revised text, project evidence and literature.
