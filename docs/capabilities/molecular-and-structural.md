...
- `alphafold3-complex-prediction` defaults to an official AlphaFold Server import package with chain mapping and manual-submission instructions. An agent may inspect access state and take the user to the official page; Google sign-in, entity review, and final submission remain user actions. The workbench stores no password, token, cookie, or browser session and calls no undocumented submission interface.
- A complete downloaded Server archive is validated as a stream. Each job must contain a request plus consistently numbered structure, summary-confidence, and full-confidence files. Every model enters the ranking and confidence tables; a deterministic top-ranked model supplies structure coordinates, residue pLDDT, binned PAE, cross-chain residue profiles, and candidate-contact tables. Only files needed for the formal deliverable are extracted, so large MSA and template directories are not expanded. An independent R renderer then creates PDF, SVG, and 600-dpi PNG figures with complete replot tables.
- Module version `1.2.0` has representative live acceptance on one completed Server download containing two jobs with five reconciled models per job. Outputs were reloaded and no credentials were persisted. This establishes executable Server-result import and figure generation for the observed archive; it is not public-data acceptance and does not convert a prediction into biological interaction evidence. See [`alphafold3-server-live-verification.json`](../../reports/alphafold3-server-live-verification.json).
- Server-origin outputs retain source tags and are blocked from downstream automated uses restricted by the official output terms. The optional local official AlphaFold 3 entry is enabled only when compatible Linux/NVIDIA hardware, approved weights, complete databases, CPU, available memory, free disk, and free GPU memory satisfy the gate while leaving at least half of current headroom unused, and the user explicitly approves deployment. The current Apple-silicon host has no compatible NVIDIA GPU, so no local model or database download and no local AlphaFold 3 inference were performed.
