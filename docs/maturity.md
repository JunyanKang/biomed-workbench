...
Externally returned outputs first pass one of 18 explicitly implemented semantic families. This establishes container readability, identity, accounting and family-level invariants only. Protocol `2.1.0` records a complete structured result and digest for every manifest gate. Family admission is never copied into assay, design, robustness, reload-scope or claim gates, and gate names never imply an automatic evaluator. A gate remains `requires_review` until a dedicated evaluator is implemented or an exact gate-level adjudication binds the receipt, output port, result digest and evidence digest. `not_evaluable` requires an explicit caveated adjudication. Artifact review, scientific decision, node completion and evidence-map identity all require the complete accepted adjudication set; caller-supplied verdicts are not accepted.
