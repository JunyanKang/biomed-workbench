...
Analysis environments use the separate `1.0.0` identity protocol. New local execution receipts bind the environment manager and name, interpreter and platform, lock-file digests, package-inventory digest, and container-image digest. New external handoffs also freeze previously accepted environment-content digests. Repeat execution compares the environment at the entry gate: exact or content-equivalent identities are reused, while drift or a legacy receipt without environment provenance blocks before computation. Private project records retain the verifiable identity without exposing machine-local paths in research state. Packaged templates must never install dependencies while analysing data.

Rerun and method-switch actions use a registry-validated child plan prepared after artifact review and before the scientific decision. Its node-level revision contract freezes the source and target nodes, action, source and target manifest digests, typed port mappings, observed and planned request identities, the complete mapped target-parameter object, scientific equivalence class, claim-scope transition, and rationale. Every output from one producing node must share that contract, action, target, and request identity. A same-method rerun preserves the observed request identity; an adjusted rerun freezes a different identity after command-parameter normalization. A method switch must use an explicit `revision_alternatives` relation with validated input, output, additional-artifact, and parameter mappings. `contract-equivalent` relations require identical quality-gate identities; decision-role alternatives reset source-specific gate conclusions and require the target gate set to be independently observed and reviewed; scope downgrades explicitly narrow the claim. Ordinary `alternatives` remain routing or recommendation relationships. Untouched downstream nodes are recreated against the mapped replacement outputs, and the replacement requires a new analysis admission before execution. Public FastQC scenarios cover both adjusted rerun and FastQC-to-fastp method-sensitivity switching through actual command execution and artifact reload.

Project state schema v2 records migration provenance. A digest-valid v1 state is replayed into v2 only when every added gate field can be recovered exactly from its original receipt; the migration retains the source digest and event count. Map-bound v1 states use the separate `migrate-state-v1` path, which verifies the immutable publication store and writes a distinct v2 file without overwriting the old state. Contract-1.1.0 v2 states use `upgrade-state-migration-1-1`; the prior state and migration/recovery digests, event chain, immutable store entry, plan-node contract, and true map coverage must all validate before a distinct 1.2.0 successor is replayed. The successor migration stores the prior state digest, prior migration digest, transition, and reason. The command reports exact scientific-dependency blockers. If prior admission was not serialized, a `historical-unavailable` recovery record documents the gap without claiming approval; it is bound to the migration source state, one verified legacy map, and that map's explicit covered or not-covered status. The recovered hypothesis and expected-output contract must match the migrated plan node exactly. This record is accepted only for project snapshots and permanently blocks delivery. Delivery prerequisites use three states: `not-assessed` when no node was requested, otherwise `satisfied` or `blocked` only after the normal delivery validator runs for every requested node. The public regressions cover v1 migration, 1.1.0-to-1.2.0 upgrade, review and decision recovery, next-map publication, immutable-store verification, and current-state reload.
