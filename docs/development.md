...
Every `agent_generated` workflow additionally requires one versioned `observed_output_contract` per output port. Container and family admission prove that the returned tables, archives, HDF5 objects, figures, structures, identities, accounting and primary bytes satisfy their frozen family rules. Manifest quality gates are a separate layer: each gate is assigned once to an evidence-capable port and classified as payload-derived, tool-native, provenance/design, system provenance or claim-boundary evaluation. A system gate may pass only from facts recomputed by the ingest path; a design or claim gate that lacks an independent evaluator is recorded as `requires_review`, never promoted from family admission. Gate receipts preserve `passed`, `failed`, `requires_review` or `not_evaluable`, the declared evidence role, its exact digest and the reason. `tools/add_observed_output_contracts.py` regenerates protocol `2.0.0`; new modules require family fixtures and gate-level negative fixtures before release.
