...
Source assimilation has a separate release gate. Ignored development ledgers hold one content identity and one clean-room design decision per inspected file. `tools/reconcile_sources.py` rejects duplicates and non-bijective ledgers, emits a private per-file reconciliation ledger, and publishes only status totals plus a canonical receipt-root digest. A `rewrite_capability` or `redesign_schema` decision stays `pending` until a private path-free receipt binding names either independent modules whose current compatibility rows, regression tests, and representative executions all pass, or digest-bound project contract evidence for non-module surfaces such as the Codex plugin manifest. Project evidence must bind the current artifact digest to a current path-free verification report; it cannot substitute for scientific module execution evidence. Unknown modules or project evidence, duplicate or absent receipts, bindings to non-pending records, and stale evidence fail reconciliation. Nonzero pending receipts explicitly prohibit a source-union completeness claim; they do not block publishing truthful incremental releases.

Private source dispositions are updated only through explicit reviewed policy. `tools/refine_source_design_ledger.py` currently recognizes the product boundary excluding CPU/GPU, containers, Slurm, remote execution, environment provisioning, and local-model infrastructure; it leaves every scientific record untouched. `tools/apply_source_capability_bindings.py` consumes ignored closed-schema match rules, rejects overlap and non-pending matches, writes deterministic receipt bindings atomically, and publishes only path-free counts. Reconciliation remains the authority that rejects a binding unless every named module has current passing compatibility, regression, and end-to-end evidence.

Public biomedical databases share `PublicJSONClient` only for transport safety: HTTPS host allow-listing, bounded bodies, transient retry policy, JSON-root validation, and request metadata. Crossref/Europe PMC citation identity, bioRxiv version history, PubChem chemical identity, ClinicalTrials.gov cohort retrieval, and RCSB structure metadata keep separate parsers, contracts, outputs, and quality gates. ClinicalTrials.gov 1.1 is count-verified and server-filtered: opaque page tokens are followed to a declared cap, unique NCT IDs reconcile with `totalCount`, capped cohorts are marked truncated, same-site location constraints use grouped Essie semantics, no hidden local post-filtering is allowed, and every request is retained in provenance.
...
GitHub CI is a repository-quality boundary, not a scientific inference module. It runs the complete test and release suite, rebuilds deterministic evidence, rejects generated drift, and performs a checksum-verified redacted full-history secret scan. Scientific eval thresholds and domain-specific gold sets remain separate named evidence requirements and are never inferred from a green generic CI job or a nondecreasing test count. Source review-window governance is implemented by the independently routed `source-freshness-audit` module; actual upstream drift still requires separate recorded upstream verification and is never inferred from snapshot age or CI status.

When a later product-boundary decision changes a private per-file disposition, rebuild the public path-free decision summary with `tools/build_rewrite_design_summary.py` before reconciliation. CPU/GPU provisioning, container orchestration, Slurm and remote-cluster configuration/submission/monitoring, and local-model hosting are classified as `retire` even when a source supplies executable preflight or operational guidance; only separable scientific-analysis contracts may be rewritten as modules.
