...
Private source dispositions are updated only through explicit reviewed policy. `tools/refine_source_design_ledger.py` currently recognizes the product boundary excluding CPU/GPU, containers, Slurm, remote execution, environment provisioning, local-model inference, and non-biomedical materials-science operations; it leaves every in-scope scientific record untouched. `tools/apply_source_capability_bindings.py` consumes ignored closed-schema match rules, rejects overlap and non-pending matches, writes deterministic receipt bindings atomically, and publishes stable path-free bindings-per-rule counts. Reconciliation remains the authority that rejects a binding unless every named module has current passing compatibility, regression, and end-to-end evidence.
