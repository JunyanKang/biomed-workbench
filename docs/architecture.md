...
- Optional credentials must remain in the project allowlist and be assigned to exact endpoint contracts. `NCBI_API_KEY` is currently the only stored credential and is optional for the implemented NCBI E-utilities and Datasets endpoints. Browser-authenticated services use a separate secret-free access-state record: AlphaFold Server records status and terms review but never a password, token, cookie, or session. Private or paid services require separate future contracts rather than reuse by database name.
