...
The case runs MAFFT `7.526` and IQ-TREE `3.1.2` with model `LG+G4`, one thousand ultrafast bootstrap replicates, fixed seed `17`, and the declared yeast record as an outgroup-presence check. The acceptance gates require all four records in the alignment and tree, exact tip accounting, and a reloadable Newick tree.
