...
For academic language revision, run `academic-prose-revision-audit` before editing, revise the same scientific content and declared structure, then run it again on the original and revised text. Remove formulaic openings, promotional significance, novelty padding, empty intensifiers, vague attributions, citation dumping, redundant connectors, ornamental vocabulary, overlong clause stacks, unsupported overclaiming, and em dashes. Preserve evidence-tied hedging, passive voice where appropriate, first-person plural, formal definitions, technical terms, numbers, results, equations, symbols, and citations. Match an author sample and venue register when supplied. Return the revised text with a short change report and an explicit preservation result. This workflow improves scholarly clarity; it must not be used to evade authorship or AI-use disclosure.

For grant and fellowship writing, do not apply paper-style restraint mechanically to the proposal's vision. Run `research-proposal-quality-audit` to freeze the scope, research canon, evidence table, argument map, section contracts, aims, feasibility evidence, fallbacks, and agency review criteria. Ambition is acceptable when the supplied preliminary evidence, prior work, collaborator, method, or staged risk control makes it credible. Never invent preliminary results, prior funding, partners, letters, facilities, approvals, or agency requirements. Use `academic-prose-revision-audit` in `grant-proposal` mode only after the scientific foundation is complete.
