...
class ModulePackagingTests(unittest.TestCase):
    def test_checked_module_index_exactly_matches_all_builtin_manifests(self):
        ...
        self.assertGreaterEqual(len(registry.all()), 48)
    ...
    def test_every_builtin_module_passes_strict_packaging_validation(self):
        ...
        self.assertEqual(by_id["citation-resolution-adjudication"]["executed_test_cases"], 2)
        self.assertEqual(by_id["classification-gold-set-evaluation"]["executed_test_cases"], 1)
    ...
    def test_release_validator_enforces_module_registry_and_no_central_intent_tables(self):
        ...
        self.assertIn(f"modules={len(ModuleRegistry.discover(BUILTIN_ROOT).all())}", result.stdout)
