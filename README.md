...
当前注册表包含 **194 个科学模块**。能力首先按数据尺度区分为 bulk、single-cell、spatial 与跨尺度通用方法，再按测量家族和工具角色细分；靶标、抗体、内部参照、特异性处理和归一化始终是实验或分析参数，不会被注册成新的组学门类。模块注册表示科学契约已经建立；真实验收范围仍以公共数据案例中登记的后端、版本、数据与重新读取的产物为准。
