...
External scientific engines remain project dependencies rather than bundled vendor code. The plugin guides, validates, and invokes them, and Codex may create a bounded project-local or temporary environment when a required package is absent, install the declared scientific versions, and run compatibility plus representative execution checks. It never silently upgrades the user's existing environment and does not provision CPUs, GPUs, containers, schedulers, remote compute, or local models. Versions in the table are reproducibility baselines observed in real verification runs, not installation pins for every project. Each module separately declares a conservative compatibility policy and records the actual detected versions in execution provenance.
...
- `reports/public-database-live-verification.json`: live Crossref, Europe PMC, bioRxiv, PubChem, ClinicalTrials.gov v2, RCSB PDB, and AlphaFold DB evidence plus isolated module-package validation.
