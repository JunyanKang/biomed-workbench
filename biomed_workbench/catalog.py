...
def _register_builtins() -> None:
    definitions = (
        ...
        Capability(
            ...
            workflow="runtime",
            kind="command",
            title="Run an approved local container",
            description="Execute a validated Docker or Podman command after permission and persist a secret-free run manifest.",
            entrypoint="biomed_workbench.capabilities.runtime:container_run",
            input_schema=_object_schema(
                {
                    "image": {"type": "string", "minLength": 3},
                    "command": {"type": "array", "items": {"type": "string"}, "minItems": 1},
                ...
                ("image", "command", "output_directory"),
            ),
            requirements=("docker or podman",), access="local_runtime", mutability="writes_output",
        ),
        Capability(
            id="slurm-plan",
            workflow="runtime",
        ...
        Capability(
            ...
            workflow="runtime",
            kind="command",
            title="Submit an approved Slurm job",
            description="Persist a validated batch script, submit it through sbatch after permission, and record the scheduler response.",
            entrypoint="biomed_workbench.capabilities.runtime:slurm_submit",
            input_schema=_object_schema(
                {
                    "command": {"type": "array", "items": {"type": "string"}, "minItems": 1},
                    "job_name": {"type": "string", "minLength": 1},
                ...
                ("command", "job_name", "cpus", "memory_gb", "time_minutes", "output_directory"),
            ),
            requirements=("Slurm sbatch",), access="local_runtime", mutability="submits_job",
        ),
        Capability(
            id="slurm-monitor",
            workflow="runtime",
            kind="command",
            title="Monitor a Slurm job",
            description="Query a numeric Slurm job identifier and normalize queue state, elapsed time, nodes, and query failures.",
            entrypoint="biomed_workbench.capabilities.runtime:slurm_monitor",
            input_schema=_object_schema(
                {
                    "job_id": {"type": "string", "pattern": "^[0-9]+(?:_[0-9]+)?$"},
                    "timeout_seconds": {"type": "integer", "minimum": 1, "maximum": 300},
                },
                ("job_id",),
            ),
            requirements=("Slurm squeue",), access="local_runtime", mutability="read_only",
        ),
        Capability(
            id="local-model-plan",
            workflow="molecular_design",
        ...
        Capability(
            ...
            workflow="molecular_design",
            kind="command",
            title="Run an approved local scientific model",
            description="Execute a license-gated local model, confine outputs, hash produced artifacts, and persist a secret-free manifest.",
            entrypoint="biomed_workbench.capabilities.runtime:local_model_run",
            input_schema=_object_schema(
                {
                    "backend": {"type": "string", "enum": ["boltz", "foldseek", "mmseqs", "proteinmpnn", "diffdock"]},
                    "inputs": {"type": "object"},
                ...
                ("backend", "inputs", "output_directory"),
            ),
            requirements=("registered local scientific model",), access="local_runtime", mutability="writes_output",
        ),
        Capability(
            id="expression-qc",
