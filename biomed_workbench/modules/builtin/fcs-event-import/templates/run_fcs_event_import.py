#!/usr/bin/env python3
"""Project template for importing a complete FCS event table.

Adapt only the request construction after inspecting the actual FCS artifact.
This template preserves the module's bounded JSON handoff: it does not sample,
compensate, transform, gate, or interpret events during import.
"""
...
MODULE_ID = "fcs-event-import"
MODULE_VERSION = "1.0.0"
ENTRYPOINT = "biomed_workbench.capabilities.quantitative_assays:import_fcs_events"
INPUT_ARTIFACT_PORTS = ("fcs_file",)
OUTPUT_ARTIFACT_PORTS = ("events",)
QUALITY_GATE_IDS = ("fcs-import-completeness", "fcs-import-interpretation-boundary")
...
def validate_request_shape(request: dict[str, Any]) -> None:
    ...
    if bool(parameters.get("fcs_path")) == bool(parameters.get("fcs_base64")):
        raise ProjectTemplateError("parameters must bind exactly one FCS source")
...
def validate_result(result: dict[str, object]) -> None:
    ...
    if tuple(result.get("quality_gate_ids", ())) != QUALITY_GATE_IDS:
        raise ProjectTemplateError("result does not retain the complete quality-gate binding")
    if not isinstance(result.get("result"), dict) or not isinstance(result.get("provenance"), dict):
        raise ProjectTemplateError("result or provenance is missing after import")
    if not result.get("request_digest"):
        raise ProjectTemplateError("request digest is missing after import")
