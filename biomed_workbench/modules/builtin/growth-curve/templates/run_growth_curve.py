#!/usr/bin/env python3
"""Project template for diagnostic-bound bacterial growth-curve fitting."""
...
MODULE_ID = "growth-curve"
MODULE_VERSION = "2.0.0"
QUALITY_GATE_IDS = (
    "growth-curve-input-contract",
    "growth-curve-fit-diagnostics",
    "growth-curve-replication-boundary",
)
INPUT_ARTIFACT_PORTS = ("growth_measurements",)
...
def validate_request(request: dict[str, Any]) -> None:
    ...
    if "observations" not in parameters:
        raise ProjectTemplateError("request must preserve replicate-level observations")
    ...
    if ports != set(INPUT_ARTIFACT_PORTS) or len(ports) != len(artifacts):
        raise ProjectTemplateError("request must bind one unique growth-measurements artifact")
...
def validate_result(result: dict[str, object]) -> None:
    ...
    if not isinstance(summary, dict) or not isinstance(result.get("provenance"), dict):
        raise ProjectTemplateError("result or provenance is missing after fitting")
    required = {"fit_admissible", "interpretation_status", "selected_model", "model_candidates", "timepoint_summary"}
    if not result.get("request_digest") or not required.issubset(summary):
        raise ProjectTemplateError("growth-curve result lacks its request binding or diagnostics")
